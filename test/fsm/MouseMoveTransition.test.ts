...
describe("using a mouse move transition", () => {
    ...
    test("valid event", () => {
        expect(tr.accept(createMouseEvent("mousemove", canvas, 11, 43, 12, 11, 1))).toBeTruthy();
    });

    test("guard OK", () => {
        expect(tr.guard(createMouseEvent("mousemove", canvas, 11, 23, 12, 25, 1))).toBeTruthy();
    });

    test("accepted events", () => {
        expect(tr.getAcceptedEvents()).toStrictEqual(new Set(["mousemove"]));
    });
});
