...
beforeEach(() => {
    tr = new MouseUpTransition(new StdState(mock<FSMImpl<FSMDataHandler>>(), "a"), new StdState(mock<FSMImpl<FSMDataHandler>>(), "b"));
...
test("guard OK", () => {
    expect(tr.guard(createMouseEvent("mouseup",
        canvas, 11, 23, 12, 25, 1))).toBeTruthy();
});
