...
beforeEach(() => {
    tr = new TouchReleaseTransition(new StdState(mock<FSMImpl<FSMDataHandler>>(), "a"), new StdState(mock<FSMImpl<FSMDataHandler>>(), "b"));
...
test("guard OK", () => {
    expect(tr.guard(createTouchEvent("touchend", 3,
        canvas, 11, 23, 12, 25))).toBeTruthy();
});
