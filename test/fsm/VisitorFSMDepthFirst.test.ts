...
describe("using the std FSM visitor implementation", () => {
    ...
    test("visiting the touch Dnd FSM works", () => {
        ...
        expect(visitor.res).toBe(">i[init]-touchstart-s[touched]-touchstart-s[touched]-touchend-c[cancelled]-touchmove-s[moved]-touchmove-s[moved]-touchmove-c[cancelled]-touchstart-s[touched]-touchend-c[cancelled]-touchend-t[released]-touchstart-c[cancelled]-touchmove-c[cancelled]-touchstart-c[cancelled]-touchmove-s[moved]");
    });
    ...
    test("visiting the multi-touch FSM works", () => {
        ...
        visitedFSM.acceptVisitor(visitor);
        // eslint-disable-next-line max-len
        const subFSM = ">i[init]-touchstart-s[touched]-touchstart-s[touched]-touchend-t[released]-touchmove-s[moved]-touchmove-s[moved]-touchmove-c[cancelled]-touchstart-s[touched]-touchend-c[cancelled]-touchend-t[released]-touchmove-c[cancelled]-touchmove-s[moved]";
